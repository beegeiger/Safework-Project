...
class Forum(db.Model):
	...
	parent_forum_id = db.Column(db.Integer, nullable=True, db.ForeignKey('forums.forum_id'))
...
class Post(db.Model):
	...
	parent_post_id = db.Column(db.Integer, nullable=True, db.ForeignKey('posts.post_id'))
...
class User(db.Model):
	...
	picture = image_attachment('UserPicture', nullable=True)
...
class Incident(db.Model):
	...
	police_dept_id = db.Column(db.Integer, db.ForeignKey('police.police_dept_id'))
	...
	cop_pic = image_attachment('CopPicture', nullable=True)
	...
	def __repr__(self):
        ...
        return "<incident_id={} police_dept_id={} source_id={} user_id={} inc_type={} latitude={} longitude={} city={} state={} datetimr={} description={} police_rec_num={} cop_name={} cop_badge={} cop_desc={} cop_pic={} sting_strat={} avoidance={} other={}>".format(
        	self.incident_id, self.police_dept_id, self.source_id, self.user_id, self.inc_type, self.latitude, self.longitude, self.city, self.state, self.datetime, self.description, self.police_rec_num, self.cop_name self.cop_badge, self.cop_desc, self.cop_pic, self.sting_strat, self.avoidance, self.other)
